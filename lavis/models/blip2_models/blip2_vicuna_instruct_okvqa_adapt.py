...
from lavis.models.adapt_module.t5_wrapper import T5ForwardWrapper
from lavis.models.adapt_module.amortized_encdec import AmortEncDecAggregateWrapper
from lavis.models.adapt_module import get_amortize_encdec_model
...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        parser = argparse.ArgumentParser(description="Amort")
        parser.add_argument("--cfg-path",  required=True,  help="path to configuration file.")
        parser.add_argument("--pretrained_model_amort", default="../public/t5-base")
        parser.add_argument(
            "--options",
            nargs="+",
        ...
        parser.add_argument("--num_virtual_tokens", default=24)
        parser.add_argument("--load_path", default=None)
        parser.add_argument("--token_dim", default=768)
        parser.add_argument("--lift_ratio", default=1.0)
        parser.add_argument("--question_model_amort", default=None)
        parser.add_argument("--layer_num_virtual_tokens", default=2)
        parser.add_argument("--base_num_layers", default=2)
        parser.add_argument("--base_model_torch_dtype", default=None)
        parser.add_argument("--base_num_attention_heads", default=2)
        parser.add_argument("--no_aggregate", default=False)
        parser.add_argument("--dropout_p", default=0.0)
        parser.add_argument("--num_cross_attention_blocks", default=4)
        parser.add_argument("--hierarchy_aware", default=False)

        args = parser.parse_args()


        args.base_num_layers = self.llm_model.config.num_hidden_layers
        args.base_num_attention_heads = self.llm_model.config.num_attention_heads
        args.token_dim  =  self.llm_model.config.hidden_size
        
        # args.base_model_torch_dtype = self.llm_model.base_lm.dtype

        args.cfg_path = "lavis/projects/blip2/train/okvqa_ft.yaml"
        # amort_model_cfg = Config(args)

        self.base_model_prepare_inputs_for_generation = self.llm_model.prepare_inputs_for_generation
        # 
        ...
        # cfg = amort_model_cfg.get("amort", None)
        self.amort_model = get_amortize_encdec_model(args)
        self.num_virtual_tokens = args.num_virtual_tokens
        self.layer_num_virtual_tokens = args.layer_num_virtual_tokens
    ...
    def forward(self, samples):
        ...
        ######################################################################################
        update_batch = {}
        update_batch["text_ids"] = knowledge_tokens.input_ids
        update_batch["text_attention"] = knowledge_tokens.attention_mask
        update_batch["text_ids_amort"] = knowledge_tokens.input_ids
        update_batch["text_attention_amort"] = knowledge_tokens.attention_mask
        update_batch["gen_q_ids_amort"] = text_input_tokens.input_ids
        update_batch["gen_q_attn_mask_amort"] = text_input_tokens.attention_mask

        prompts, context_summary_bank = self.amort_model(update_batch)
        
        # concat prompt attention mask
        prefix_attention_mask = torch.ones(bs, self.layer_num_virtual_tokens).to(attention_mask.device)
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            ######################################################################################
            self.llm_model.prepare_inputs_for_generation = self.prepare_inputs_for_generation_peft

            text_input_tokens = llm_tokens
            update_batch = {}
            update_batch["text_ids"] = knowledge_tokens.input_ids
            update_batch["text_attention"] = knowledge_tokens.attention_mask
            update_batch["text_ids_amort"] = knowledge_tokens.input_ids
            ...
            # concat prompt attention mask
            # prefix_attention_mask = torch.ones(self.input_num, self.layer_num_virtual_tokens).to(attention_mask.device)
            ...
            if num_beams > 1:
                ...
                for prompt in prompts:
                    new_prompts.append(prompt.repeat(1, num_beams, 1, 1, 1))
                prompts = tuple(new_prompts)

            # prompts = DynamicCache.from_legacy_cache(prompts)
    ...
    def prepare_inputs_for_generation_peft(self, input_ids=None, past_key_values=None, attention_mask=None, inputs_embeds=None, prompts: torch.Tensor = None, **kwargs):
        model_kwargs = self.base_model_prepare_inputs_for_generation(input_ids=input_ids, past_key_values=past_key_values, attention_mask=attention_mask, inputs_embeds=inputs_embeds, **kwargs)
        ...
        if uses_cache and (model_kwargs["past_key_values"] is not None) and "input_ids" in model_kwargs.keys():
        ...
        if model_kwargs.get("attention_mask", None) is not None:
            size = model_kwargs["attention_mask"].shape[0], self.layer_num_virtual_tokens
            prefix_attention_mask = torch.ones(size).to(model_kwargs["attention_mask"].device)
