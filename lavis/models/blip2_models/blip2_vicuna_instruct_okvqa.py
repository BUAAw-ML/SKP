...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        self.weight1_proj1 = nn.Linear(
            self.llm_model.config.hidden_size, 250
        )
        ...
        self.activate = torch.nn.LeakyReLU()   #LeakyReLU() # Sigmoid() #ReLU()    #Tanh() #GeLU() #GELU()
    ...
    def forward(self, samples):
        ...
        mean_sim_score = (sim_score * knowledge_tokens['attention_mask'].unsqueeze(-1)).mean(-1).sum(-1) / knowledge_tokens['attention_mask'].sum(-1) 
...
#######################################
    ...
    def generate(
        ...
        #######################determine output##########################
        answer_dict = {}
        answer_dict[output_text[self.knowledge_num]] = -30
    ...
    def from_config(cls, cfg):
        ...
        # model.query_tokens2 = model.query_tokens
        # model.query_tokens3 = model.query_tokens
